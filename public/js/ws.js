...
function Connection(path) {
    var self = this;
    this.path = path;
    var wsock = window.WebSocket;
    if(!wsock && window.MozWebSocket) {
        wsock = window.MozWebSocket;
    }
    if(wsock) {
        if(window.location.protocol == 'http:') {
            this.conn = new wsock('ws://' + location.host + path);
        } else {
            this.conn = new wsock('wss://' + location.host + path);
        }
        this.conn.onerror = function(ev) { self.http_fallback(ev); }
        // TODO(tailhook) implement timeout
    } else {
